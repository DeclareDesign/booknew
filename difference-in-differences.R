...
design <- 
  ...
  declare_potential_outcomes(Y ~ X + 0.5 * as.numeric(period) + D + U,
                             assignment_variable = D) + 
  declare_estimand(ATE = mean(Y_D_1 - Y_D_0), subset = period == 2) + 
  declare_step(D = X == max(X), handler = mutate) + 
  reveal_outcomes(Y = if_else(D == 0 | period == 1, Y_D_0, Y_D_1), 
                  handler = mutate) +
  declare_estimator(Y ~ D * period, model = lm_robust, se_type = "none")
