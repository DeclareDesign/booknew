...
Randomized saturation designs can be used to measure the diffusion treatment effects throughout a pre-defined area or network (@bairdetal2018). The design works in two phases. First, groups of units are cluster-assigned to treatment *saturations*, e.g. 0%, 25%, and 75% saturation. Then, in each group, units are individually assigned in proportions determined by the saturations: e.g., in the groups randomly assigned to 0%, none of the units are assigned to treatment, while in the groups assigned to 25% and 75%, one-quarter and three-quarters of the units are assigned to treatment, respectively. By comparing untreated units in the 0% groups to their *untreated* counterparts in the higher-saturation groups, researchers can estimate the indirect treatment effects that result from saturation-induced spillovers.

### Declaration

- **M**odel: There are ten groups each with one hundred units. Units' potential outcomes are defined in terms of `S`---the unit's group's saturation---and `Z`---whether or not a unit is treated. 
...
- Increase saturation in low group to .25
