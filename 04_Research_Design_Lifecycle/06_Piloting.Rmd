...
rds_file_path <- paste0(get_dropbox_path("06_Piloting.Rmd"), "/sims.RDS")
if (do_diagnosis & !exists("do_bookdown")) {
  write_rds(sims, path = rds_file_path)
}
sims <- read_rds(rds_file_path)
```

```{r pilotingfig, fig.cap = "Learning from pilot studies.", fig.width = 6.5, fig.height = 3}
gg_df <- sims %>% 
  mutate(
