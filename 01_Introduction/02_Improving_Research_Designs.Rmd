...
In an experiment in Pakistan, @Gulzar2020 study how to motivate citizens to run for political office for the first time. We declare a simplified version of their design to illustrate design declaration in code. In Appendix XX we declare the complete design.
...
simple_design <- 
  ...
  declare_population(
    # 100 villages
    villages = add_level(N = 100),
    
    # 100 citizens in each village
    citizens = add_level(
      N = 50, 
      # unobserved shock 'u' with intra-class correlation of 0.1
      u = draw_normal_icc(mean = 0, sd = 1, clusters = villages, ICC = 0.1)
    )
  ) +
...
## Appendix: Detailed research design declaration of @Gulzar2020

In this section, we declare the @Gulzar2020 example in finer detail. The aim is to capture all of the analytically-relevant features of the design. We first describe the design in words and then declare in code.

The *model* describes the set of units is all citizens living in the two Pakistani districts the study took place in, Haripur and Abbottabad. Their endogenous outcomes of interest are whether the citizen filed papers to run for office; whether the citizen was elected; and the Euclidean distance of the citizen's preferences from average citizen preferences. The outcomes are a function in their causal model of three treatments, which emphasize either the social or personal benefits to holding public office or do not encourage anyone to run for office at all. Their model would include an expected treatment effect magnitude for each treatment and their suppositions about how correlated outcomes are within villages that they study. 
