...
There is, of course, no great difference in this example between `custom_assignment` and `my_assignment_step` since `my_assignment_step` is just a function that applies  `custom_assignment`. Even still by declaring this step formally as a design step using `declare_assignment` you let `DeclareDesign` know how the step fits into the whole design, how to interpret it and when to call it.   
