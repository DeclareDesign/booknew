...
kable(head(voter_file), digits = 3, caption = "Example data", booktabs = TRUE)
```
...
my_assignment_step(voter_file) %>% head %>% kable(caption = "Data generated using a custom function", booktabs = TRUE)
```
...
declare_population(data = voter_file)() %>% head %>% kable(digits = 3, caption = "Draw from a fixed population", booktabs = TRUE)
```
...
draw_data(simple_design) %>% head %>% kable(digits = 3, caption = "Sampled data.", booktabs = TRUE)
```
...
simple_design_data %>% head %>% kable(digits = 3, caption = "Data with revealed outcomes.", booktabs = TRUE)
```
...
# where you want to be able reuse the function with different variable 
...
draw_data(simple_design) %>% head %>% kable(digits = 3, caption = "Simulated data draw.", booktabs = TRUE)
```
...
simulations_df %>% kable(digits = 3, caption = "Simulations data frame.", booktabs = TRUE)
```
