...
Here are two quite common examples where a problem like this arises. First, imagine a researcher assumes that $Y= \beta_0+\beta_1 X$ and says they want to learn about $\beta_1$.  A second researcher thinks the true model is $Y= \gamma_0 + \gamma_1 X +\gamma_2 X^2$. What then is researcher 1's question under researcher 2's model? For a second example researcher 1 is interested in the average treatment effect, which they describe as  $E(Y(1)-Y(0))$. Researcher 2 wonders how well researcher 1's strategy works if there are spillovers, but then realizes that the estimand,   $E(Y(1)-Y(0))$, presupposed no spillovers. How then to assess whether the strategy works well?
