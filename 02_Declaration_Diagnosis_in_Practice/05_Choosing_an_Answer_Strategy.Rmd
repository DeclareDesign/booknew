...
Opening to the answer strategy section.

1. Let's recapitualte where we came from.  I is to M as A is to D.

2. At the highest level, an answer strategy is a conditioning strategy. Recapitulate the DAG rules. We want to close back-door paths and we want to be sure not to open them back up.

3. Both qualitative and quanti


## Choosing conditioning strategies using DAGS
...
## Qualitative answer strategies are answer strategies too

1. Process tracing.

2. QCA

(not full declaration)

## For Quant, this is the choice of estimator

[Huge list of estimators]

A good rule of thumb is the plug-in principle, which relies on the symmetry between I(m) and A(d). When you can replace expectations in the I(m) expression with summations in the A(d) expression, you're doing plug in estimation:


- I(m) : $E[Y_i(1) - Y_i(0)] = E[Y_i(1)] - E[Y_i(0)]$
- A(d) : $\frac{1}{m}\sum_1^m Y_i - \frac{1}{N-m}\sum_{N-m}^N Y_i$
