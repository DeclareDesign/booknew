...
design <-
  declare_model(N = 250, gender = rep(0:1, N/2), Y_star = 1 + gender + 2* rnorm(N)) +
  declare_inquiry(Y_bar = mean(scale(Y_star)[gender == 1])) + 
  ...
  declare_estimator(Y_av ~ 1, model = lm_robust, inquiry = "Y_bar", subset = gender ==1, label = "Average") +
  declare_estimator(Y_fa ~ 1, model = lm_robust, inquiry = "Y_bar", subset = gender ==1, label = "principal components")
