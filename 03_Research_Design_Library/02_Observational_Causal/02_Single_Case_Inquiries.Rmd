...
Note here that model used for $M$ and $M$ is the same model used for $A$. This does not *have* to be the case however. If instead we used different models in these two parts we could assess how well or poorly our strategy performs when our model is wrong. In this case, and unlike the results in table   \@ref(tab:ptdiagnosis) we woudl find that hte expected posterior variance (where the expectation is taken with respect to the model in $M$ but posterior taken with respect to the model in $A$) will not be the same as the expected mean squared error. Can you see why not?
