...
```{r}
fixed_population <-
  fabricate(N = 500, Y = sample(1:7, N, replace = TRUE))
...
fixed_pop <-
  fabricate(
    ...
    subject = 
      add_level(N = N_i_in_cluster,
                latent = draw_normal_icc(mean = 0, 
                                         N = N, 
                                         clusters = cluster, 
                                         ICC = icc),
...
kable(reshape_diagnosis(diagnosis), digits = 2, booktabs = TRUE)
```
