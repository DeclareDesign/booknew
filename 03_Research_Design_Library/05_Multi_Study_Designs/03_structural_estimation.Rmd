...
```{r structural-estimation, echo = FALSE, output = FALSE, purl = FALSE}
...
Structural estimation is used in situations where researchers have a general model in mind for how processes work and their goal is fit the parameters of the model. With the fitted model they might then estimate levels of unobserved variables, treatment effects, or other quantities. They might even extrapolate to estimate counterfactual quantities, such as the effects of interventions that have not been implemented [@reiss2007structural]. 
...
Here  $y = \sum_{j = 2}^n(-1^{j})\delta^{j-1}$ is the equilibrium offer made by the first mover as predicted by the @rubinstein1982perfect alternating offers bargaining model with $n$ possible bargaining rounds.  $\delta$ is the discount factor. The  customer's payoff  depends on whether she goes first ($z_i = 1$) or second ($z_i = 0$). Non-rational customers ($\theta_i = 0$) insist on giving the driver some share $\chi$ of their endowment, irrespective of whether they go first or second. We let $q$ denote the probability that $\chi = 1$.
...
We imagine that $Z$ is randomly assigned and we have access to data on payments, $\pi$. Our goal however is not simply to measure the effect of $Z$ on $\pi$ but to estimate the model parameters which themselves can be used to estimate this effect and other counterfactual quantities.
...
- **I**nquiries. Our inquiries will include parameters `k`, `d`, and `q` corresponding to $\kappa$, $\delta$ and $q$ in the model (we will treat $\chi$ as known). In addition we will be interested in the effect of $Z$ in a two period game and in the game of indefinite duration.  
...
- **A**nalysis is implemented using maximum likelihood to identify which parameter values are most consistent with the data (which collection of parameter values produce the observed data with greatest likelihood). In this declaration the model employed in **A** is the same as that employed in **M**. We will report analysis results implemented both using the $n=2$ data (`DIM_two, Struc_two`) and using $n=\infty$ data (`DIM_inf, Struc_inf`) 
