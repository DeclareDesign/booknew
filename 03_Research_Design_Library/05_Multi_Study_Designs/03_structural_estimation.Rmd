...
Wilke and Humphreys (2020) imagine a bargaining game with payoffs to $i$ given by:

$$\pi_i =  \theta_i(z_i\omega + (1-z_i)(1-\omega)) + (1-\theta_i)\phi + u_3$$

Here  $\omega = \sum_{j = 2}^n(-1^{j})\delta^{j-1}$ is the equilibrium offer made by the first movee as predicted by the @rubinstein1982perfect alternating offers bargaining model with $n$ possible bargaining rounds.  $\delta$ is the discount factor. The  customer's payoff  depends on whether she goes first ($z_i = 1$) or second ($z_i = 0$). Non-rational customers ($\theta_i = 0$) insist on giving the driver some share $\phi$ of their endowment, irrespective of whether they go first or second. The last term, $u_3$, is a random disturbance.
