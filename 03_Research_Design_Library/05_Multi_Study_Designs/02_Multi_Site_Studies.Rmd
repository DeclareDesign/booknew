...
single_site_design <- 
  ...
  declare_estimand(PATE_feasible = mean(Y_Z_1 - Y_Z_0), subset = feasible_site == FALSE) + 
...
multi_site_design <- 
  ...
  declare_potential_outcomes(Y ~ Z * (0.1 + study_effect + feasible_effect * feasible_site) + noise) +
  declare_estimand(PATE = mean(Y_Z_1 - Y_Z_0)) + 
  declare_estimand(PATE_feasible = mean(Y_Z_1 - Y_Z_0), subset = feasible_site == FALSE) + 
  declare_sampling(clusters = site, strata = feasible_site, strata_n = c(0, n_study_sites)) + 
  declare_sampling(strata = site, n = n_subjects_per_site) + 
  declare_assignment(blocks = site, prob = 0.5) + 
  declare_estimator(handler = random_effects_meta_analysis)
