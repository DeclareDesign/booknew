...
When there are heterogeneous effects, you can get good predictions out of sample even when average effects differ substantially (and you do better with multiple sites when sites in the the population have different proportions of subject types that are correlated with heterogeneous effects). 

Two notable features of the design:
- there must be heterogeneous effects for this to work (otherwise our estimates get biased toward zero due to overfitting to the het variables).
- we have to have information about the covariate in the population and the sample (here we used the proportion of people in each heterogeneous types).
...
design <- 
  declare_population(
    ...
    declare_estimator(handler = label_estimator(post_stratification), pr_types_population = pr_types, label = "post-strat")

single_site_large_design <- redesign(design, n_study_sites = 1, n_subjects_per_site = 2500)

small_study_five_sites <- redesign(design, n_study_sites = 5, n_subjects_per_site = 500)
```
...
```{r, echo = FALSE}
kable(get_diagnosands(diagnosis_small_large) %>% reshape_diagnosis %>% select(Bias, RMSE), booktabs = TRUE)
```

### Bayesian estimation can improve estimates of effects for sampled sites

To do: you can improve site-level effect estimates by analyzing with simple Bayesian model because of its shrinkage property, even when the Bayesian model is wrong about distribution of effects in population.
