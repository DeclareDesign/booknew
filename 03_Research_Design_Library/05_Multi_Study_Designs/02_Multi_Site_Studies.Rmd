...
multi_site_designer <- function(
  ...
  declare_population(
    ...
    declare_estimator(handler = tidy_estimator(post_stratification), pr_types_population = pr_types, label = "post-strat")
}
...
```{r, echo = FALSE}
kable(get_diagnosands(diagnosis_small_large), booktabs = TRUE)
```
...
```{r, echo = FALSE}
kable(get_diagnosands(diagnosis_feasible_effects), booktabs = TRUE)
```
