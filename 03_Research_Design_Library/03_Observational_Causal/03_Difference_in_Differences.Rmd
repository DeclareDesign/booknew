...
design <- 
  ...
  reveal_outcomes(Y = if_else(Z == 0 | period == 1, Y_Z_0, Y_Z_1), 
                  handler = mutate) +
...
two_period_two_group_design <- 
  ...
  declare_potential_outcomes(
    Y_Z_0 = unit_shock + 0.5 * time + unit_time_shock, 
    Y_Z_1 = Y_Z_0 + 1) +
  
  declare_estimand(ATE = mean(Y_Z_1 - Y_Z_0), subset = time == 1) + 
  
  declare_assignment(
    Z = unit_shock == max(unit_shock), 
    handler = mutate
  ) + 
  
  reveal_outcomes(
    Y = case_when(Z == 0 | time < 1 ~ Y_Z_0, TRUE ~ Y_Z_1), 
    handler = mutate) +
  
  declare_estimator(
    estimate = (mean(Y[Z == 1 & time == 1]) - 
                  mean(Y[Z == 0 & time == 1])) -
...
diagnosis_two_period_two_group <- diagnose_design(
  two_period_two_group_design, diagnosands = declare_diagnosands(bias = mean(estimate - estimand)),
...
```{r}
kable(get_diagnosands(diagnosis_two_period_two_group), booktabs = TRUE)
```
...
multi_period_design <- 
  ...
  declare_potential_outcomes(
    Y_Z_0 = unit_shock + 0.5 * time + noise, 
    Y_Z_1 = Y_Z_0 + 0.2) +
  
  declare_estimand(ATE = mean(Y_Z_1 - Y_Z_0), subset = time == 1) + 
  
  declare_assignment(Z = 1*(unit_treated & pretreatment == FALSE), 
                     handler = fabricate) + 
  reveal_outcomes(Y, Z) + 
...
```{r}
kable(get_diagnosands(diagnosis_multi_period_multi_group), booktabs = TRUE)
```

- Show that in case where some units switch back and forth between T and C during panel there is bias (point to Imai and Kim with weighted FE estimator to fix this)
