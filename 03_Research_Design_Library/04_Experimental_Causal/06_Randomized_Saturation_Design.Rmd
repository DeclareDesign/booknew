...
```{r}
design <- 
  declare_population(network = add_level(N = 4), 
                     unit = add_level(N = 4, U = rnorm(N))) +
  declare_assignment(assignment_variable = "S", 
                     clusters = network, 
                     conditions = c("low","high")) +
  declare_assignment(prob = 0.25, 
                     blocks = network, 
                     assignment_variable = "Z_S_low") +
  declare_assignment(prob = 0.75, 
                     blocks = network, 
  ...
  declare_potential_outcomes(Y ~ Z + spillover_low * (S == "low") + 
                               Z * spillover_high * (S == "high") + U,
  ...
  reveal_outcomes(Z,S) +
  declare_step(ipw = 1 / (
    S_cond_prob * (Z_S_low_cond_prob * (S == "low") + 
  ...
  declare_estimator(
    ...
    model = lm_robust,
    term = c("Z", "Shigh"),
    estimand = c("ate_no_spill", "ate_saturation"),
    weight = ipw
  )
```
...
- Setting the saturations back to their original values, remove the `ipw` from the estimator. What happens to the bias? Explain your answer.
...
design <- 
  declare_population(N = N_individuals, 
                     X = 1:N, 
                     U = rnorm(N),
                     G = ntile(X, N_groups)) + 
  declare_assignment(assignment_variable = "S", 
...
```{r}
diagnosis %>% reshape_diagnosis() %>% kable(booktabs = TRUE)
```
...
```{r}
distal_diagnosis %>% reshape_diagnosis() %>% kable(booktabs = TRUE)
```
