...
The highest level advice we have about redesign is, at the beginning of the processes at least, change one design parameter at a time. Vary the data strategy, holding the model, inquiry, and answer strategy constant. Change the answer strategy, while holding the other aspects of the design constant. Graphical representation of how diagnosands change as designs change are an intuitive way to do this.

## Formal optimization

* Example with a loss function drawn from a decision problem and selection of an optimal design over some set. Candidate: assume a decision will be taken if estimate is of a given size. Costs of different types of errors. Note: optimization does not have to use diagnosands as inputs but may be done via a complex diagnosand. Optimization may take account of priors over $M$.
