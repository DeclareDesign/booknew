...
utility <- declare_diagnosands(
  utility_2 = mean((statistic > 1.96)*(estimate > c)*(estimand - c) - N/100000+ r/2500))
...
Payoffs across two decision dimensions are show in Figure \@ref(fig:optimaldesign).
...
diagnosis <- diagnose_design(designs, sims = 5000)
```
...
optimal_design_diagnosis$simulations_df %>% group_by(N, r) %>% summarize(
  utility_2 = mean((statistic > 1.96)*(estimate > c)*(estimand - c) - N/100000  + .0004*r)) %>% 
  ggplot(aes(N, 4 - r, z = utility_2)) + 
  ylab("Investment in precision") + geom_contour_filled(show.legend = FALSE) + scale_x_continuous(trans = 'log10')
