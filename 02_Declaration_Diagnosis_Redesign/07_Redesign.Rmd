...
utility <- declare_diagnosands(
  utility_1 = mean((statistic > 2)*(estimate > c)*(estimand - c)),
  utility_2 = mean((statistic > 2)*(estimate > c)*(estimand - c) - N/20000+ r/50)
)
                                  
design <- set_diagnosands(design, utility)

designs <- redesign(design, N = c(100, 500, 1000), r = c(0, .5, 1))
```
...
rds_file_path <- paste0(get_dropbox_path("02_Improving_Research_Designs"), "/optimal_design.RDS")
if (do_diagnosis & !exists("do_bookdown")) {
  write_rds(diagnosis, path = rds_file_path)
}
optimal_design_diagnosis <- read_rds(rds_file_path)

# plot as a contour plot showing optimal design ; should be an elipse
