...
The $M$ in MIDA refers to these reference models.
...
Which population you have in mind affects how reference populations should be modeleld but also how draws from a population should be interpreted. 

To illustrate consider a very simple model in which we believe that $Y$ depends upon $X$ and $X$ is *as if* randomly assigned, with probability .5. Say we believe that potential outcomes, and treatment effects, are different for each person. This model, like many in this books, might be declared like this:
...
Note now that this declaration has a stochastic component. Not only is `X` randomized but so are individual level features `U` and treatment effects `tau`. How should we interpret this stochastic component?  Note also that the "data" contains potential outcomes---that is, it includes a description of how the world would work under different conditions and not simply a description of possibly observable data. In other words it reports a data generating process, not just data. 

What type of population have we just modelled? Have we modelled one or many populations? 

In fact there are three distinct ways to interpret this model specification.

1. **Large Population Interpretation of M**: We have a large population---perhaps a superpopulation. On each run of the model we get to see a new sample and we use the sample to learn about the large population. 

2. **Multiple Worlds Interpretation of M**: A second interpretation is that each data draw is itself a (non stochastic) model and the distribution functions provide a handy way of generating multiple plausible models. We we will want to learn about design performance for each of these possible worlds.
...
Which interpretation we have matters for who we do diagnosis. A point we return to in section XXX.
...
The key components in all the reference models we examine are the "variables" or "nodes"  and the ways these stand in relation to each other. Informally a variable is a quantity that can take on different values: the day of the week, the winner of an election, the height of a mountain. All variables are formally similar but they can play distinguished roles in a model.
...
Defining a reference model requires both defining a set of variables (their meaning and their ranges) and stipulating how they relate to each other. In the graphical modelling literture we define a set of nonparametric structural causal relationships, indicating which variables depend on which other variables. With these variables in hand, we can  visualize the model using a directed acyclic graph (DAG).
...
The justification of your choice of reference model will depend on the purpose of your design. Broadly we distinguish between reality tracking models, discurisve models, and sufficient models. 
