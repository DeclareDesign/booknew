...
We display the DAG for the design in Figure \@ref(p2gulzarkhandag).
...
dat_head %>% kable(digits = 3, caption = "Six rows of simulated data from the Gulzar-Khan design.")
```

From this data, we present simulated estimates of the two inquiries as well as the simulated estimand values in Table \@ref(gulzarkhansimestimates):
...
estimates$estimates_df %>% left_join(estimates$estimands_df) %>% select(estimator_label, estimate, std.error, estimand)  %>% kable(digits = 3, caption = "Simulated estimates and estimand from one run of the Gulzar-Khan design.")
```

In Table \@ref(gulzarkhansimcrosstab), we display the number of people and villages in each treatment assignment and sampling condition to illustrate the data strategy:

```{r gulzarkhansimcrosstab, echo = FALSE}
summary_df %>% 
  kable(caption = "Count of units by treatment assignment and sampling status.")
```
