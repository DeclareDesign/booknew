...
We let $a^m$ denote the answer to $I$ \textit{under the model}. Conditional on the model, $a^m$ is the value of the estimand, the quantity that the researcher wants to learn about. The connection of $a^m$ to the model can be seen in the following equality: $I(m) = a^m$.

The answer under the model ($a^m$) may be close or far from the true value of the inquiry, which is to say it could be wrong. If the model $M$ is far from the truth, then of course $a^m$ need not be correct. We denote the true answer to $I$ as $a^w$: the answer to $I$ that depends on the true state of the world $w$. We note that $a^w$ might be undefined if the model wrong enough.
...
| $w = M()$     | the real world $w$ is a draw from the true causal model $W$                           |
