...
Answer strategies commonly rely on measures of uncertainty like $p$-values, standard errors, and confidence intervals in order to make decisions about how to interpret $a^d$. Any measure estimated from the data to make a decision about $a^d$ can be used as a diagnostic statistic and summarized as a diagnosand. Like bias, statistical power is an expectation, this time of the diagnostic statistic $\mathbb{1}(p \leq 0.05)$, an indicator function that equals 1 if the $p$-value is no greater than 0.05 and 0 otherwise. Power describes how frequently (under beliefs about the model) a research design would return a statistically significant result. A standard error provides another diagnostic statistic whose expectation provides the expected standard error diagnosand, for example. This can be especially informative in comparison with another diagnosand, $\sqrt{\Var(a^d)}$, the actual standard deviation of the estimates generated by the model.

Some diagnosands can be calculated analytically. It can be straightforward to calculate $\sqrt{\Var(a^d)}$ if you have a two-arm experiment and are willing to make a lot of simplifying assumptions.^[Insert formula 3.4 and cite to FEDAI.] Most diagnosands for even moderately complex designs, however, require Monte Carlo computer simulation. A main purpose of the `DeclareDesign` software package is making the simulation step easier.
