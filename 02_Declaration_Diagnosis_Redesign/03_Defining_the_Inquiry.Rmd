...
Distinct from the notion of a causal effect is that of causal attribution. A causal effect inquiry focuses on the change in an outcome that would be induced by a change in the causal variable (at the unit-level or across units)---it is defined for every unit, irrespective of the values that the outcome takes. By contrast, causal attribution inquiries focus on conditional probabilities, such as, the "probability of the absence of the outcome in the hypothetical absence of the treatment ($Y_i(0) = 0$) given the actual presence of both ($X_i = Y_i = 1$)" [@yamamoto2012understanding, pp.240-241]. @pearl1999 refers to such inquiries as X, using the prime sign to denote values of the outcome and treatment variable incompatible with their observed values. @goertz2012tale refer to such inquiries as cause-of-effects questions because they start with an outcome (an effect) and seek to find its cause, e.g. "'Was economic crisis necessary for democratization in the Southern Cone of Latin America?' or 'Were high levels of foreign investment in combination with soft authoritarianism and export-oriented policies sufficient for the economic miracles in South Korea and Taiwan?'" [@goertz2012tale]. 



| Inquiry                                                  | Description                                                                    |
| -------------------------------------------------------- | ------------------------------------------------------------------------------ |
| $Pr(Y'_i(X_i')  \mid X_i,~ Y_i)$                           | Probability $X$ necessary for $Y$                                              |
| $Pr(Y_i(X_i)  \mid X'_i,~ Y'_i)$                           | Probability $X$ sufficient for $Y$                                              |
| $Pr(Y_i(0) = 0 \mid X_i = Y_i = 1, X_i(z)=z)$            | Complier probability of causal attribution                                     |
| $Pr(Y_i(0) = 0 \mid X_i = Y_i = 1, X_i(z)=z)$            | Complier probability of causal attribution                                     |


<!-- For example, consider the alternative causal quantity E(Yi (1) − Yi (0) | Xi = Yi = 1), which we may call the average treatment effect on the treated and successful. The probability of causal attri- bution is equal to one minus the second term in this expression. I exploit such mathematical relations in my nonparametric identifi- cation analysis below. -->

Qualitative comparative analysis (QCA) with boolean variables ("crisp-set QCA"), for example, focuses on finding the minimal set of conditions sufficient to produce $Y$. If $Y$ happens any time that causal factor $A$ is present or when $A$ is absent but $B$ is present, then $a^M$ is a set: $\{A, \not A \& B\}$. Process-tracing designs often focus on cause-of-effects questions like whether the presence of a strong middle class caused a revolution (@goertz2012tale). The inquiry could be written $\Pr(Y_i(X_i=0)=0| X_i=1, Y_i(X_i=1)=1)$, which in words means the probability that $Y$ would not have happened had $X$ not happened, given that $Y$ and $X$ both happened in fact. 
