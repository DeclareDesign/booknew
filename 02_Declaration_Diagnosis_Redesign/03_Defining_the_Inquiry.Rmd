...
Atomic inquiries $I$ about a model $M$ fall into two basic categories, descriptive and causal. Descriptive inquiries are about how the world was, is, and will be whereas causal inquiries are about how the world would have been, would be, or would be in the future if some variables were set to different levels. 
...
draw_estimands(M+I) %>% 
  kable(caption = "Estimand is a descriptive quantity.")
...
draw_estimands(M+I) %>% 
  kable(caption = "Estimand is a treatment effect")
...
In the example below we ask an attribution data of this form but on continuous data. Use the same reference model $M$ and ask about the share of units with positive values of $Y$ and $Z=1$ that would have had negative values of $Y$ were $Z=0$.

```{r}
I <- declare_estimand(POC = mean((Y_Z_0<0)[Z==1 & Y_Z_1>0]))

draw_estimands(M+I) %>% 
  kable(caption = "Estimand is the probability of necessity for the sign of Y.")

```


### Other data-dependent inquiries
