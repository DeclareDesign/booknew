...
Descriptive and causal inference have in common the difficulty of inferring unseen things from observed data. In the descriptive case, we want to learn about the fundamentally unobservable latent variables the measured variables are meant to represent. In the causal case, we want to learn about the fundamentally impossible to observe difference between two possible worlds. We can't measure $Y$ in both the $X = 1$ and $X = 0$ because only one of those values could ever be revealed by the world. This problem is commonly known as the fundamental problem of causal inference. The fundamental problem of descriptive inference is similar -- we don't get to observe the true latent values because they are latent. We can only ever observe measured variables.


### Descriptive estimands

Descriptive estimands are usually about latent variables. We care about the true values of the variables in the models. No matter how excellent the measurements taken as part of the Data strategy, the measured variables are always distinct from the latent variables since measurement always involves error. For the most part, we define our inquiries in terms of the "true" latent variables rather than in terms of their measured counterparts.
...
Causal inquires involve a comparison of at least two possible worlds. For example, an inquiry might be the causal effect of $X$ on $Y$ for a single unit. In order to infer that causal effect, we would need to know the value of $Y$ in two worlds: one world in which $X$ is set to 1 and one in which $X$ is set to 0. You can think of causal inference as repeated descriptive inference: we have to describe $Y$ in multiple possible worlds.
...
<!-- MH -- suggest here and elsewhere we hold back on pushing on experimental designs; the focus here  here is on  I not A. People might be coming at this from a bounds a pproach, a Bayes approach, etc. Or may have other strategies we haven;t thought of. Here lets keep focus on I -->
...
<!-- Again seem to be focused a bit on A rather than I.  A lot to be said about these I's though ; e.g. use of cross worl counterfactuals-->
...
<!-- WOuld like to go back to look at text we had in an earlier version of paper that classified different types of estimand -->
...
Data are real. We have to spend time *making* them. That's what empiricists do (with many different measurement procedures). They make data real by creating them. Data come in many forms, there are many things to know about creating data. There are many ways to summarize them.
...
<!-- Not sure boring/trivial versus exiciting/vital cuts it! Wonder if there are desiderata here we can point to: -->
<!-- welfare implications -->
<!-- implications for understanding past research -->
<!-- expected updating (e.g. in Bayes: expected posterior variance) (though here leaning into D/A rathr than I) -->
...
<!-- Would much rather a more positive approach here where rather than cautioning against we highlight complexity and  give guidance on how this might be done. Feels though that this shold be in A not I anyhow?-->



#### Things to incorporate

- "model" inquiries in which I'm trying to rule out some Ms.  Process tracing is 
